...
const rawExams: Record<string, Omit<ExamWeek, 'exercisesByLevel'>> = {
  exam_01: {
    id: 'exam_01',
    title: 'Exam Week 01',
    description: 'Piscine exam week 1. Start with simple output programs and argument handling, progress to basic C functions.',
    levelCount: 8,
  ...
  exam_02: {
    id: 'exam_02',
    title: 'Exam Week 02',
    description: 'Piscine exam week 2. Covers string manipulation, numeric operations, binary bit manipulation.',
    levelCount: 8,
    gradePerLevel: ((0 + 1) / 8) * 100,
    timeMinutes: 240,
  ...
  exam_03: {
    id: 'exam_03',
    title: 'Exam Week 03',
    description: 'Piscine exam week 3. Advanced C functions and string patterns.',
    levelCount: 4,
  ...
  exam_04: {
    id: 'exam_04',
    title: 'Exam Week 04',
    description: 'Piscine exam week 4. The complete C piscine exam bank, levels 0 through 5: output basics, string algorithms, bit manipulation, recursion, and linked lists.',
    levelCount: 6,
    gradePerLevel: ((0 + 1) / 6) * 100,
    timeMinutes: 240,
    exercises: exam04Data as ExamExercise[],
  },
}

function buildExercisesByLevel(exercises: ExamExercise[]): Record<number, ExamExercise[]> {
...
export const examWeeks: Record<string, ExamWeek> = {
  exam_01: { ...rawExams.exam_01, exercisesByLevel: buildExercisesByLevel(rawExams.exam_01.exercises) },
  exam_02: { ...rawExams.exam_02, exercisesByLevel: buildExercisesByLevel(rawExams.exam_02.exercises) },
  exam_03: { ...rawExams.exam_03, exercisesByLevel: buildExercisesByLevel(rawExams.exam_03.exercises) },
  exam_04: { ...rawExams.exam_04, exercisesByLevel: buildExercisesByLevel(rawExams.exam_04.exercises) },
}
...
export function getRandomExercise(week: ExamWeek, level: number, exclude: string[] = []): ExamExercise {
  ...
  if (filtered.length === 0) {
    throw new Error(`All exercises at level ${level} for ${week.id} have already been passed. Consider re-enabling passed exercises in settings.`)
  }
  const idx = Math.floor(Math.random() * filtered.length)
  return filtered[idx]
}

export function getExercise(weekId: string, level: number, name: string): ExamExercise | null {
  const week = examWeeks[weekId]
  if (!week) return null
  return week.exercises.find((e) => e.level === level && e.name === name) || null
}
...
export function getExerciseByName(weekId: string, name: string): ExamExercise | null {
  const week = examWeeks[weekId]
  if (!week) return null
...
export function getCurrentGrade(gradePerLevel: number, currentLevel: number): number {
...
export function isLevelAccessible(week: ExamWeek, level: number, currentLevel: number): boolean {
