...
async function runInSandbox(sandbox: Sandbox, cmd: string, timeoutMs = 5000): Promise<{ stdout: string; stderr: string; exitCode: number }> {
  try {
    const result = await sandbox.commands.run(cmd, { timeoutMs, onStdout: () => {}, onStderr: () => {} })
...
export async function gradeSubmission(exercise: ExamExercise, studentCode: string): Promise<GradeResult> {
  ...
  try {
    ...
    const refCompileCmd =
      exercise.type === 'function' ? 'gcc -o /tmp/ref /tmp/ref.c /tmp/main.c -Wall -Wextra -Werror 2>&1' : 'gcc -o /tmp/ref /tmp/ref.c -Wall -Wextra -Werror 2>&1'
    ...
    for (const tc of exercise.testCases) {
      ...
      try {
        ...
        results.push({ passed, input: tc.args.join(' ') || '(no args)', expected, actual })
      } catch (err) {
        results.push({
          passed: false,
...
export async function runExamExercise(exercise: ExamExercise, studentCode: string): Promise<string> {
  const apiKey = process.env.E2B_API_KEY
  if (!apiKey) {
