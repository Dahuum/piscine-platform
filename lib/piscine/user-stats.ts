...
export async function getUserStats(): Promise<UserStats | null> {
  ...
  const [progress, exams] = await Promise.all([
    supabase().from('normal_days_progress').select('module_id, exercise_id, status').eq('user_id', userId),
  ...
  const bestExamGrade = examRows.length ? Math.max(...examRows.map((e) => e.final_grade || 0)) : 0
  const passedExams = examRows.filter((e) => e.result === 'completed').length
  const passRate = totalExams ? Math.round((passedExams / totalExams) * 100) : 0
  const totalDurationSeconds = examRows.reduce((s, e) => s + (e.duration_seconds || 0), 0)
  ...
  for (const e of examRows) {
    ...
    weekMap[e.week_id].best = Math.max(weekMap[e.week_id].best, e.final_grade || 0)
  }
  ...
  return {
    ...
    passRate,
    totalHours,
    examWeeks: Object.entries(weekMap).map(([weekId, data]) => ({ weekId, ...data })),
