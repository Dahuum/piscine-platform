...
// once, at signup/login). Every piscine page reads its state from
// localStorage only — none of them query Supabase directly. Without this
// step, a user who signs in on a browser/device where localStorage is empty
// (a new device, or one that was cleared) sees an account with stats but no
// visible progress, even though the rows are safely in the database.
import { getAllModuleProgressWithCode, getAllPrepReviews, getAllPrepExercises, getExamHistory } from './db'
