...
import { gradeSubmission } from "@/lib/exam-corrector";

// Practice-mode grading for the exam prep flow — same real reference-code
// + test-case comparison the live timed exam uses (lib/exam-corrector.ts),
// just without any of the session/cooldown/scoring machinery in
// /api/exam/grade, since this isn't a graded attempt. Exists so prep can
// tell a genuinely correct submission from one that merely ran without
// crashing (what /api/execute's exam branch does) — the prep page marks an
// exercise "done" only on a real pass from here.
