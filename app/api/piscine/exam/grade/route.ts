...
import { getSession, updateSession, isTimeExpired, isCooldownActive, getCooldownRemaining } from '@/lib/piscine/exam-session'
import { getExamWeek, getExercise, getRandomExercise, isExamComplete } from '@/lib/piscine/exam-data'
import { gradeSubmission, getCooldownSeconds } from '@/lib/piscine/exam-corrector'
import type { LevelHistoryEntry } from '@/lib/piscine/exam-session'
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (session.status !== 'active') {
      return NextResponse.json({ error: 'Exam is not active', status: session.status }, { status: 400 })
    }

    if (isTimeExpired(session)) {
      session.status = 'timeout'
      await updateSession(token, session)
      return NextResponse.json({ error: 'Time has expired', status: 'timeout' }, { status: 400 })
    }

    if (isCooldownActive(session)) {
      return NextResponse.json({ error: 'Cooldown active', cooldownRemaining: getCooldownRemaining(session) }, { status: 429 })
    }

    const exercise = getExercise(session.weekId, session.currentLevel, session.currentExercise)
    ...
    if (!code.trim()) {
      return NextResponse.json({ error: 'No code to grade' }, { status: 400 })
    }

    if (!exercise) {
      const week = getExamWeek(session.weekId)
      const newEx = getRandomExercise(week, session.currentLevel, session.successEx)
      session.currentExercise = newEx.name
      await updateSession(token, session)
      return NextResponse.json({
        error: 'Exercise not found, new one assigned',
        exercise: { name: newEx.name, level: newEx.level, type: newEx.type, subject: newEx.subject },
      })
    }

    const result = await gradeSubmission(exercise, code)

    const timeOnLevel = Math.round((Date.now() - session.levelStartedAt) / 1000)
    ...
    if (result.passed) {
      ...
      if (isExamComplete(session.accumulatedPoints)) {
        ...
        const finalGrade = Math.min(100, session.gradePerLevel * session.currentLevel)
        return NextResponse.json({ passed: true, examComplete: true, finalGrade, levelHistory: session.levelHistory })
      }

      let nextExercise
      try {
        const week = getExamWeek(session.weekId)
        nextExercise = getRandomExercise(week, session.currentLevel, session.successEx)
      ...
      return NextResponse.json({
        ...
        newExercise: { name: nextExercise!.name, level: nextExercise!.level, type: nextExercise!.type, subject: nextExercise!.subject },
      ...
      session.cooldownUntil = Date.now() + getCooldownSeconds(session.assignmentCount) * 1000
      ...
      const existingFailure = session.levelHistory.find((h) => h.level === session.currentLevel && h.exercise === session.currentExercise)
      if (existingFailure) {
