...
import { getExamWeek, getRandomExercise, getExercise } from '@/lib/piscine/exam-data'
import { createSession, updateSession, getActiveSessionForVisitor, isTimeExpired, getTimeRemaining } from '@/lib/piscine/exam-session'
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!weekId || !mode || !visitorId) {
      return NextResponse.json({ error: 'Missing weekId, mode, or visitorId' }, { status: 400 })
    }

    if (mode !== 'editor' && mode !== 'terminal') {
      return NextResponse.json({ error: 'Mode must be "editor" or "terminal"' }, { status: 400 })
    }
    ...
    if (existing && !isTimeExpired(existing.session)) {
      ...
      const exercise = getExercise(weekId, session.currentLevel, session.currentExercise)
      if (exercise) {
        return NextResponse.json({
          token,
          resumed: true,
          exercise: { name: exercise.name, level: exercise.level, type: exercise.type, subject: exercise.subject },
    ...
    const { token, session } = await createSession(weekId, mode as 'editor' | 'terminal', visitorId, gradePerLevel, exercise.name)
