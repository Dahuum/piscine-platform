...
import {
  Trophy, Clock, Target, Zap, BookOpen, GraduationCap,
...
export default function ProfilePage() {
    ...
    return (
      <div className="max-w-lg mx-auto px-4 py-16 text-center">
        ...
        <p className="text-sm text-muted-foreground mb-6">
          Sign in to track your progress, save exam results, and sync across devices.
        </p>
  ...
  const noData = !stats || (stats.totalExercises === 0 && stats.totalExams === 0);
      ...
      {noData ? (
        <div className="text-center py-12 border rounded-xl">
          ...
          <p className="text-sm text-muted-foreground">
            No data yet. Start practicing or take an exam!
          </p>
          <div className="flex justify-center gap-3 mt-4">
            ...
            <Link href="/exam">
              <Button variant="outline" size="sm">Exam Gate</Button>
            </Link>
          </div>
        </div>
      ) : (
        <>
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3 mb-8">
            {[
              { icon: Target, label: "Exercises", value: String(stats!.totalExercises) },
              { icon: BookOpen, label: "Modules", value: `${stats!.modulesCompleted}/13` },
              { icon: GraduationCap, label: "Exams", value: String(stats!.totalExams) },
              { icon: Trophy, label: "Best", value: `${stats!.bestExamGrade}%` },
              { icon: Clock, label: "Hours", value: `${stats!.totalHours}h` },
                ...
                {stats!.recentExams.map((exam) => {
                  ...
                  return (
                    <div key={exam.id} className="flex items-center gap-3 px-4 py-2.5 rounded-lg border text-sm">
                      ...
                      <span className={`text-[10px] uppercase font-medium ${exam.result === "completed" ? "text-emerald-600" : "text-amber-600"}`}>
                        {exam.result}
                      </span>
                    </div>
                  );
                  ...
                  return (
                    <div key={ew.weekId} className="rounded-lg border p-3">
                      ...
                      <div className="flex items-center gap-3 mt-1 text-xs text-muted-foreground">
                        <span>{ew.attempts} attempts</span>
